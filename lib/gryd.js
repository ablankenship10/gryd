...
var Logger = Bunyan.createLogger({name: "GrydProcess."+grydconfig.env});
  ...
  fs.readdir(app_path, function (err, files) {
    if (err) {
      ...
      if (cluster.isMaster) {
        ...
        initialize(GlobalApp, app_path, files, function (Apps) {
          for (var i in Apps) {
            var App = Apps[i];
...
function initialize(GlobalApp, app_path, files, callback) {
  var Apps = [];
